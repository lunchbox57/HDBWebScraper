...
class HDBWebScraper:
    ...
    def __init__(self, master):
        ...
        # self.pcode_file_dir_label = tk.Label(text="Enter postal code:")
        self.postal_file_dir_text = tk.StringVar()
        self.postal_file_dir_label = tk.Label(master, textvariable=self.postal_file_dir_text)
        self.pcode_file_dir_input = tk.Button(text="Choose excel file", command=self.get_postal_code_file)
        ...
        # self.pcode_file_dir_label.grid(row=3, column=1, padx=50)
        self.postal_file_dir_label.grid(row=4, column=1, padx=50)
        self.pcode_file_dir_input.grid(row=5, column=1, padx=50)
    ...
    def get_postal_code_file(self):
        ...
        if len(path) > 0:
            self.postal_file_dir_text.set(path)

    def search(self):
        self.notif_text.set("")
        pcode_list = self.read_file(self.postal_file_dir_text.get())
        ...
        with pd.ExcelWriter(path=workbook_name, engine='xlsxwriter') as writer:
            ...
            for pcode in pcode_list:
                addr_resp, lease_resp, price_details_resp = self.query_onemap(pcode)
                if addr_resp.status_code != 200 or lease_resp.status_code != 200 or price_details_resp.status_code != 200:
                    self.populate_excel_with_error(pcode, writer, f"An error occurred (code: {lease_resp.status_code}). Please inform dev.")
                elif len(addr_resp.json()['results']) == 0:
                    self.populate_excel_with_error(pcode, writer, f"Postal Code {pcode} is invalid")
                else:
                    addr, df, lease_duration, lease_start_date, remaining_lease = self.extract_data(addr_resp,
                                                                                                    lease_resp,
                                                                                                    price_details_resp)
                    self.populate_excel(addr, df, lease_duration, lease_start_date, pcode, remaining_lease, workbook,
                                        writer)
    ...
    def populate_excel(self, addr, df, lease_duration, lease_start_date, pcode, remaining_lease, workbook, writer):
